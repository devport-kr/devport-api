...
class WikiChatServiceQualityTest {
    ...
    fun chatResultExposesClarificationAndContinuitySeams() {
        val result =
            WikiChatResult(
                "요약하면 인증은 JWT 필터 중심이에요.",
                true,
                listOf("인증 흐름", "권한 체크", "리프레시 토큰"),
                listOf("SecurityConfig 경로를 알려줘", "JWT 필터 역할을 설명해줘"),
                true,
                false,
            )
    ...
    fun returnsGroundedSummaryBeforeOneNarrowClarification() {
        val result =
            WikiChatResult(
    ...
    fun returnsShortKoreanClarificationBulletsForAmbiguousQuestions() {
        val result =
            WikiChatResult(
    ...
    fun stopsClarificationLoopAfterTwoTurns() {
        val result =
            WikiChatResult(
                "요약하면 현재 근거로는 JwtAuthenticationFilter 중심 흐름까지가 가장 안전해요.",
                false,
                emptyList(),
                listOf("SecurityConfig 경로를 알려줘", "JWT 필터 호출 순서를 설명해줘"),
                true,
                false,
            )
    ...
    fun weakGroundingRepliesStayShortAndSuggestBetterNextQuestions() {
        val result =
            WikiChatResult(
                "요약하면 지금은 deploy.yml 근거만 확인돼요.",
                false,
                emptyList(),
                listOf("deploy.yml 기준으로 단계별로 설명해줘", "배포 secret 목록을 알려줘", "롤백 흐름을 설명해줘"),
                false,
                false,
            )
